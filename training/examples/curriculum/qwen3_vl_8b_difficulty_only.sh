#!/usr/bin/env bash
# Curriculum experiment — DIFFICULTY-ONLY (Table 7 row "Difficulty").
# Data is the merged corpus, sorted by sample difficulty (easy → hard) and
...
python3 -m verl.trainer.main \
    config="${VLMCC_CONFIG}" \
    data.train_files="${VLMCC_DIFFICULTY_MERGED_TRAIN}" \
    ...
    trainer.experiment_name=qwen3_vl_8b_curriculum_difficulty_only \
